<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="post" action="{{ isset($post) ? route('posts.update', $post->id) : route('posts.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        ...
                        <div>
                            <x-input-label for="cover_image" value="Featured Image" />
                            <label class="block mt-2">
                                <span class="sr-only">Choose image</span>
                                <input type="file" id="cover_image" name="cover_image" accept=".jpg, .jpeg, .png" class="block w-full text-sm text-slate-500
                            ...
                            <div class="shrink-0 my-2">
                                <img id="cover_image_preview" class="h-64 w-128 object-cover rounded-md" src="{{ isset($post) ? Storage::url($post->cover_image) : '' }}" alt="Featured image preview" />
                            </div>
    ...
    <script>
        // create onchange event listener for cover_image input
        document.getElementById('cover_image').onchange = function(evt) {
            const [file] = this.files
            if (file) {
                // if there is an image, create a preview in cover_image_preview
